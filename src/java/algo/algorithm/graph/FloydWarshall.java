...
public class FloydWarshall {
    ...
    /**
     * In computer science, the Floyd–Warshall algorithm is an algorithm for finding shortest paths in a weighted graph
     * with positive or negative edge weights (but with no negative cycles).[1][2] A single execution of the algorithm
     * will find the lengths (summed weights) of shortest paths between all pairs of vertices. Although it does not
     * return details of the paths themselves, it is possible to reconstruct the paths with simple modifications to the
     * algorithm. Versions of the algorithm can also be used for finding the transitive closure of a relation
     * {\displaystyle R} R, or (in connection with the Schulze voting system) widest paths between all pairs of vertices
     * in a weighted graph.
     * <p>
     * Worst-case performance	    O(V^3) Best-case performance	    O(V^3) Average performance	        O(V^3) Worst-case
