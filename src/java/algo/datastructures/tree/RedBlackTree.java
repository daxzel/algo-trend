 ...
 * <p>
 * Average	    Worst Space	    O(n)	    O(n) Search	O(log n)	O(log n) Insert	O(log n)    O(log n) Delete	O(log n)	O(log
 * n)
 */
class RedBlackTree {
    ...
    /**
     * Case 1: The current node N is at the root of the tree. In this case, it is repainted black to satisfy property 2
     ...
     * children, but because N is red, the paths through each of its children have the same number of black nodes as the
     * path through the leaf it replaced, which was black, and so this property remains satisfied.
     */
     ...
     * because property 4 (both children of every red node are black) is still violated. The rotation causes some paths
     * (those in the sub-tree labelled "1") to pass through the node N where they did not before. It also causes some
     * paths (those in the sub-tree labelled "3") not to pass through the node P where they did before. However, both of
     * these nodes are red, so property 5 (all paths from any given node to its leaf nodes contain the same number of
     * black nodes) is not violated by the rotation. After this case has been completed, property 4 (both children of
     ...
     * of its parent G. In this case, a right rotation on G is performed; the result is a tree where the former parent P
     * is now the parent of both the current node N and the former grandparent G. G is known to be black, since its
     * former child P could not have been red otherwise (without violating property 4). Then, the colors of P and G are
     * switched, and the resulting tree satisfies property 4 (both children of every red node are black). Property 5
     * (all paths from any given node to its leaf nodes contain the same number of black nodes) also remains satisfied,
     ...
     * grandparent. Note that P has to be black as it had a red child. The resulting subtree has a path short one black
     * node so we are not done. Now N has a black sibling and a red parent, so we can proceed to step 4, 5, or 6. (Its
     * new sibling is black because it was once the child of the red S.) In later cases, we will relabel N's new sibling
     * as S.
     */
     ...
     * through S, which are precisely those paths not passing through N, have one less black node. Because deleting N's
     * original parent made all paths passing through N have one less black node, this evens things up. However, all
     * paths through P now have one fewer black node than paths that do not pass through P, so property 5 (all paths
     * from any given node to its leaf nodes contain the same number of black nodes) is still violated. To correct this,
     * we perform the rebalancing procedure on P, starting at case 1.
     */
    ...
    /**
     * S and S's children are black, but P is red. In this case, we simply exchange the colors of S and P. This does not
     * affect the number of black nodes on paths going through S, but it does add one to the number of black nodes on
     * paths going through N, making up for the deleted black node on those paths.
     */
    ...
    /**
     * S is black, S's left child is red, S's right child is black, and N is the left child of its parent. In this case
     * we rotate right at S, so that S's left child becomes S's parent and N's new sibling. We then exchange the colors
     * of S and its new parent. All paths still have the same number of black nodes, but now N has a black sibling whose
     * right child is red, so we fall into case 6. Neither N nor its parent are affected by this transformation. (Again,
     * for case 6, we relabel N's new sibling as S.)
     */
    ...
    /**
     * Case 6: S is black, S's right child is red, and N is the left child of its parent P. In this case we rotate left
     * at P, so that S becomes the parent of P and S's right child. We then exchange the colors of P and S, and make S's
     * right child black. The subtree still has the same color at its root, so Properties 4 (Both children of every red
     * node are black) and 5 (All paths from any given node to its leaf nodes contain the same number of black nodes)
     * are not violated. However, N now has one additional black ancestor: either P has become black, or it was black
     * and S was added as a black grandparent. Thus, the paths passing through N pass through one additional black
     * node.
     */
    ...
    private static void toRed(Element element) {
        if (element != null) {
            element.toRed();
        }
    }

    private static void toBlack(Element element) {
        if (element != null) {
            element.toBlack();
        }
    }
